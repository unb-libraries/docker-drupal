...
then
  ...
  if [ -n "$MYSQL_ROOT_PASSWORD" ]; then
    ...
    PREINSTALL_SQL=$(printf "%s\n" \
      "DROP DATABASE IF EXISTS $DRUPAL_DB_NAME;" \
      "CREATE DATABASE $DRUPAL_DB_NAME CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" \
      "CREATE USER '$DRUPAL_DB_USER'@'%' IDENTIFIED BY '$DRUPAL_DB_PASSWORD';" \
      "GRANT ALL PRIVILEGES ON $DRUPAL_DB_NAME.* TO '$DRUPAL_DB_USER'@'%' IDENTIFIED BY '$DRUPAL_DB_PASSWORD';" \
      "FLUSH PRIVILEGES;")
    mysql -uroot -p"$MYSQL_ROOT_PASSWORD" -h "$DRUPAL_DB_HOSTNAME" -P "$DRUPAL_DB_PORT" -e "$PREINSTALL_SQL"
  else
